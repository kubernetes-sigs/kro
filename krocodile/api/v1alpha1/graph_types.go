...
// ExternalRefMetadata identifies the target of an ExternalRef. Exactly one of
// Name or Selector must be set.
//
...
type ExternalRefMetadata struct {
	// Name is the name of the external resource to reference. Mutually
	// exclusive with Selector.
	//
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:MinLength=1
	Name string `json:"name,omitempty"`
	// Namespace is the namespace of the external resource. For single
	// references (Name), defaults to the Graph's namespace. For collection
	// references (Selector), empty means list across all namespaces.
	//
	// +kubebuilder:validation:Optional
	Namespace string `json:"namespace,omitempty"`
	// Selector is a label selector for collection references. When set, all
	// resources matching the selector are included. Mutually exclusive with
	// Name.
	//
	// +kubebuilder:validation:Optional
...
// ExternalRef is a reference to a resource that already exists in the cluster.
// It allows reading existing resources into a Graph's scope without managing
// their lifecycle.
type ExternalRef struct {
	// APIVersion of the external resource.
	//
	// +kubebuilder:validation:Required
	APIVersion string `json:"apiVersion"`
	// Kind of the external resource.
	//
	// +kubebuilder:validation:Required
	Kind string `json:"kind"`
	// Metadata identifying the external resource.
	//
	// +kubebuilder:validation:Required
	Metadata ExternalRefMetadata `json:"metadata"`
}

// ForEachDimension defines a single expansion axis. Each entry is a map with
// exactly one key/value pair where the key is the loop variable name and the
// value is a CEL expression evaluating to an array. Multiple dimensions form a
// cartesian product.
//
...
// one of the four type-discriminating fields (Template, Ref, Watch, Def)
