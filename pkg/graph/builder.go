...
// setExpectedTypeOnDescriptor sets the ExpectedType field on a FieldDescriptor by:
// 1. Taking the path (e.g., "spec.containers[0].name")
// 2. Stripping array indices (e.g., "spec.containers.name")
// 3. Constructing type name with __type_ prefix
// 4. Looking up schema for that path
// 5. Converting schema to CEL type
func setExpectedTypeOnDescriptor(descriptor *variable.FieldDescriptor, rootSchema *spec.Schema, resourceID string) {
	// String templates always expect StringType - already set by parser
	if !descriptor.StandaloneExpression {
		return
	}

	// Strip array indices from path
	strippedPath, err := fieldpath.StripIndices(descriptor.Path)
	if err != nil {
		// If we can't parse, use DynType as fallback
		descriptor.ExpectedType = cel.DynType
		return
	}

	// Construct type name: __type_ + resourceID + . + strippedPath
	typeName := "__type_" + resourceID
	if strippedPath != "" {
		typeName = typeName + "." + strippedPath
	}

	// Look up schema for this path
	fieldSchema := rootSchema
	if strippedPath != "" {
		fieldSchema = lookupSchemaAtPath(rootSchema, strippedPath)
	}

	// Convert schema to CEL type with proper naming
	descriptor.ExpectedType = getCelTypeFromSchema(fieldSchema, typeName)
}
...
func validateNode(resource *Resource, templatesEnv, schemaEnv *cel.Env, resourceSchema *spec.Schema, typeProvider *apiservercel.DeclTypeProvider) error {
...
func validateTemplateExpressions(env *cel.Env, resource *Resource, typeProvider *apiservercel.DeclTypeProvider) error {
	for _, templateVariable := range resource.variables {
...
func validateExpressionType(outputType, expectedType *cel.Type, expression, resourceID, path string, typeProvider *apiservercel.DeclTypeProvider) error {
	...
	if typeProvider != nil && krocel.AreTypesStructurallyCompatible(outputType, expectedType, typeProvider) {
		return nil
	}
