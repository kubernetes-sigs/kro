...
func TestGoNativeType_OptionalNone_FeatureGate(t *testing.T) {
	t.Run("feature gate disabled returns nil", func(t *testing.T) {
		require.NoError(t, features.FeatureGate.Set("CELOmitFunction=false"))
		got, err := GoNativeType(types.OptionalNone)
		require.NoError(t, err)
		assert.Nil(t, got)
	})

	t.Run("feature gate enabled returns omit sentinel", func(t *testing.T) {
