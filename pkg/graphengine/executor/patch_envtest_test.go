...
// TestPatch_ForEachRegistersPerTargetDriftWatch pins finding 3901220457: a
// forEach patch must register a drift watch for EACH rendered target, so a
// manual edit or deletion of any patched target re-enqueues the Graph and the
// contribution is re-applied. Before the fix the collection patch skipped the
// watch entirely (N targets collapsed to one coordinator key), so drift on a
// fanned-out target went unobserved. Each target is watched under its own
// synthetic node ID keyed by identity, so all N survive.
func TestPatch_ForEachRegistersPerTargetDriftWatch(t *testing.T) {
	cl := patchEnvClient(t)
	ns := "default"
	mustCreateConfigMap(t, cl, ns, "drift-a", map[string]any{"orig": "a"})
	mustCreateConfigMap(t, cl, ns, "drift-b", map[string]any{"orig": "b"})

	g := generator.NewGraph("g",
		generator.WithNamespace(ns),
		generator.WithDef("src", map[string]any{"names": []any{"drift-a", "drift-b"}}),
		generator.WithPatchManifest("p", map[string]any{
			"apiVersion": "v1",
			"kind":       "ConfigMap",
			"metadata":   map[string]any{"name": "${n}"},
	...
	g.Spec.Nodes[len(g.Spec.Nodes)-1].ForEach = []expv1alpha1.ForEachDimension{{"n": "${src.names}"}}
	g.SetUID("uid-foreach-driftwatch")
	...
	// One scalar drift watch per target, each addressing its own ConfigMap.
	var patched []string
	for _, req := range w.reqs {
		if req.Selector == nil && req.GVR.Resource == "configmaps" {
			patched = append(patched, req.Namespace+"/"+req.Name)
		}
	}
	assert.ElementsMatch(t, []string{ns + "/drift-a", ns + "/drift-b"}, patched,
		"every forEach patch target must get its own scalar drift watch")
}

// TestPatch_ForEachBestEffortAcrossMissingTarget pins finding 3901216373: a
// forEach patch over ["missing", "present"] must still patch the target that
// DOES exist even though an earlier target is absent. Before the fix the first
// absent target aborted the fan-out, so "present" was never patched (and the
// result was order-dependent). Best-effort: the present target is patched, its
// contribution is recorded, and the node stays soft not-ready so the reconcile
// requeues and patches "missing" once it appears.
func TestPatch_ForEachBestEffortAcrossMissingTarget(t *testing.T) {
	cl := patchEnvClient(t)
	ns := "default"
	mustCreateConfigMap(t, cl, ns, "present", map[string]any{"orig": "p"})

	g := generator.NewGraph("g",
		generator.WithNamespace(ns),
		generator.WithDef("src", map[string]any{"names": []any{"missing", "present"}}),
		generator.WithPatchManifest("p", map[string]any{
			"apiVersion": "v1",
			"kind":       "ConfigMap",
			"metadata":   map[string]any{"name": "${n}"},
	...
	g.Spec.Nodes[len(g.Spec.Nodes)-1].ForEach = []expv1alpha1.ForEachDimension{{"n": "${src.names}"}}
	g.SetUID("uid-foreach-besteffort")

	rt := compileAndBuild(t, g)
	res, err := NewSimple(cl).Apply(context.Background(), rt, watchrouter.NoopWatcher{})

	// The absent target keeps the node soft not-ready and Unresolved (so nothing
	// is pruned), but the present target must have been patched anyway.
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrNotReady)
	assert.Contains(t, res.Unresolved, "p")
	require.Len(t, res.Contributions, 1, "the one present target is patched despite the missing sibling")
	assert.Equal(t, "present", res.Contributions[0].Name)

	cm := getConfigMap(t, cl, ns, "present")
	data, _, _ := unstructured.NestedStringMap(cm.Object, "data")
	assert.Equal(t, "yes", data["patched"], "the present target got the fanned-out contribution")
	assert.Equal(t, "p", data["orig"], "the present target's pre-existing field survives")
}
