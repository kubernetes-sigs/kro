	...
	tests := []struct {
		...
		},
		{
			...
			run: func(t *testing.T, reg *Registry) {
				...
				_, ok := reg.Get("demo-rgd", 2)
				assert.False(t, ok)
				assert.Equal(t, int64(3), mustLatest(t, reg).Revision)

				reg.Delete("demo-rgd", 3)
				assert.Equal(t, int64(1), mustLatest(t, reg).Revision)
		...
		{
			name: "deleteRevisionsBefore removes empty buckets and ignores unknown rgds",
			run: func(t *testing.T, reg *Registry) {
				reg.DeleteRevisionsBefore("demo-rgd", 3)
				_, ok := reg.Latest("demo-rgd")
				assert.False(t, ok)
		...
		},
		{
			...
			run: func(t *testing.T, reg *Registry) {
				...
				_, ok := reg.Get("demo-rgd", 1)
				assert.False(t, ok, "revision 1 should be gone")
				_, ok = reg.Get("demo-rgd", 2)
				assert.False(t, ok, "revision 2 should be gone")
				_, ok = reg.Get("demo-rgd", 3)
				assert.False(t, ok, "revision 3 should be gone")
				_, ok = reg.Latest("demo-rgd")
				assert.False(t, ok, "latest should be gone")
				assert.False(t, reg.HasAll("demo-rgd", []int64{1}), "hasAll should return false")

				entry, ok := reg.Get("other-rgd", 1)
				require.True(t, ok, "unrelated rgd must be preserved")
				assert.Same(t, one, entry.CompiledGraph)
			},
		},
		{
			name: "deleteAll is idempotent for absent rgds",
			run: func(t *testing.T, reg *Registry) {
...
func mustLatest(t *testing.T, reg *Registry) Entry {
	t.Helper()
