...
func TestNodeLabeler(t *testing.T) {
	...
	t.Run("namespaced, without collection", func(t *testing.T) {
		labeler := NewNodeLabeler(instance, true, "my-node", nil)
		assert.Equal(t, GenericLabeler{
			OwnedLabel:              "true",
			KROVersionLabel:         v,
			InternalOwnedLabel:      "true",
	...
	t.Run("cluster scoped, with collection", func(t *testing.T) {
		labeler := NewNodeLabeler(instance, false, "my-node", &CollectionInfo{Index: 2, Size: 5})
