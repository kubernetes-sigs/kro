...
### Instance Condition Metrics

Exported only when the `InstanceConditionMetrics` [feature gate](./02-feature-gates.md#instanceconditionmetrics) is enabled.

| Metric | Type | Description | Stability |
| -------- | ------ | ------------- | ----------- |
| `instance_condition_current_status_seconds` | Gauge | The current amount of time in seconds that an instance status condition has been in a specific state | ALPHA |
...
## Graph Engine Watch Metrics

Emitted by the watch router that serves the [Graph](../concepts/graph/01-overview.md) API. These series are only populated when the `GraphKind` [feature gate](./02-feature-gates.md#graphkind) is enabled.

| Metric | Type | Description | Stability |
| -------- | ------ | ------------- | ----------- |
| `graphengine_watch_owner_count` | Gauge | Number of Graphs currently tracked by the watch router | ALPHA |
| `graphengine_watch_request_count` | Gauge | Number of active watch requests, labeled by `gvr` and `kind` (`scalar` or `collection`) | ALPHA |
| `graphengine_route_total` | Counter | Total events routed through the watch router, labeled by `gvr` | ALPHA |
| `graphengine_route_match_total` | Counter | Total events that matched at least one Graph, labeled by `gvr` | ALPHA |
