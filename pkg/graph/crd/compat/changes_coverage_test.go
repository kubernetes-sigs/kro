...
func TestChangeDescriptions(t *testing.T) {
	...
	tests := []struct {
		...
		{"enum added", Change{Path: "spec.e", ChangeType: EnumAdded, NewValue: "red,green,blue"},
			[]string{"Enum constraint", "added", "red,green,blue"}},
		{"enum removed", Change{Path: "spec.e", ChangeType: EnumRemoved, OldValue: "red,green,blue"},
			[]string{"Enum constraint", "removed", "red,green,blue"}},
		...
		{"x-validation added", Change{Path: "spec.arns", ChangeType: XValidationAdded, NewValue: "self.all(item, item.matches('^arn:aws'))"},
			[]string{"CEL validation rule", "added", "self.all"}},
		{"x-validation removed", Change{Path: "spec.arns", ChangeType: XValidationRemoved, OldValue: "self.all(item, item.matches('^arn:aws'))"},
			[]string{"CEL validation rule", "removed", "self.all"}},
