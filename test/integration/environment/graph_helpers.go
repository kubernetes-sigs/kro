...
// TestingT abstracts *testing.T and GinkgoT() for helper functions.
...
func RawExt(t TestingT, v any) *runtime.RawExtension {
	t.Helper()
...
func (e *Environment) CreateNamespace(t TestingT) string {
	t.Helper()
	name := fmt.Sprintf("krcdl-%d-%s", time.Now().UnixNano(), rand.String(5))
...
func (e *Environment) CreateGraph(t TestingT, g *expv1alpha1.Graph) *expv1alpha1.Graph {
	t.Helper()
	ctx := e.context
	if ctx == nil {
		ctx = context.Background()
	}
	if err := e.Client.Create(ctx, g); err != nil {
...
// half-deleted state.
func (e *Environment) deleteGraphAndWait(g *expv1alpha1.Graph) error {
...
// typed client.
func (e *Environment) GetGraph(t TestingT, key types.NamespacedName) *expv1alpha1.Graph {
	t.Helper()
	g := &expv1alpha1.Graph{}
	ctx := e.context
	if ctx == nil {
...
func (e *Environment) AwaitCondition(
	t TestingT,
...
func (e *Environment) AwaitObject(
	t TestingT,
...
func (e *Environment) AwaitGraphGone(t TestingT, key types.NamespacedName, timeout time.Duration) {
	t.Helper()
	deadline := time.Now().Add(timeout)
	ctx := e.context
	if ctx == nil {
...
func (e *Environment) AwaitDeleted(t TestingT, gvk schema.GroupVersionKind, key types.NamespacedName, timeout time.Duration) {
	t.Helper()
	deadline := time.Now().Add(timeout)
	ctx := e.context
	if ctx == nil {
...
func (e *Environment) UpdateGraphSpec(t TestingT, key types.NamespacedName, mutate func(*expv1alpha1.Graph)) {
	t.Helper()
	ctx := e.context
	if ctx == nil {
		ctx = context.Background()
...
// informative.
func Eventually(t TestingT, timeout, interval time.Duration, fn func() error) {
	t.Helper()
	deadline := time.Now().Add(timeout)
	var last error
	for {
...
// converges to it.
func Consistently(t TestingT, duration, interval time.Duration, fn func() error) {
	t.Helper()
