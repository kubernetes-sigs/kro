...
func TestExpression_Eval(t *testing.T) {
	...
	t.Run("cost limit disabled by default allows large evaluations", func(t *testing.T) {
		ast, issues := env.Compile("[1, 2, 3, 4, 5].map(x, x * 2)")
		require.NoError(t, issues.Err())
		prg, err := env.Program(ast, DefaultProgramOptions()...)
		require.NoError(t, err)
		e := &Expression{Original: "[1, 2, 3, 4, 5].map(x, x * 2)", Program: prg}
		got, err := e.Eval(map[string]any{})
		require.NoError(t, err)
		assert.Equal(t, []any{int64(2), int64(4), int64(6), int64(8), int64(10)}, got)
	})

	t.Run("cost limit exceeded when tuned with low limit", func(t *testing.T) {
		ast, issues := env.Compile("[1, 2, 3, 4, 5].map(x, x * 2)")
		require.NoError(t, issues.Err())
		prg, err := env.Program(ast, ProgramOptions(2)...)
		require.NoError(t, err)
		e := &Expression{Original: "[1, 2, 3, 4, 5].map(x, x * 2)", Program: prg}
