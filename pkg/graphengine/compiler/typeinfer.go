...
func inferSchemaFromValue(v any) *spec.Schema {
	...
	case []any:
		...
		// Empty arrays infer to list<dyn>; non-empty arrays compare per-element
		// schemas and fall back to dyn if elements mismatch.
		if len(val) == 0 {
			return &spec.Schema{SchemaProps: spec.SchemaProps{
				Type:  []string{"array"},
				Items: &spec.SchemaOrArray{Schema: dynSchema()},
			}}
		}
		first := inferSchemaFromValue(val[0])
		mismatch := false
		for _, elem := range val[1:] {
			elemSchema := inferSchemaFromValue(elem)
			if !schemasEqual(first, elemSchema) {
				mismatch = true
				break
			}
		}
