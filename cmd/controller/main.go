...
func main() {
	...
	flag.BoolVar(&enableInstanceTelemetry, "enable-instance-telemetry", false,
		"Enable instance-level telemetry: Prometheus metrics, K8s events, and structured logs for status condition transitions. "+
			"When enabled, overrides the resync period to 60s for stuck condition detection.")
	...
	// When instance telemetry is enabled, use a shorter 60s resync to enable
	// level-based stuck condition detection. The DeepEqual guard in
	// updateStatus prevents unnecessary API writes, so this is safe.
	effectiveResyncPeriod := resyncPeriod
	if enableInstanceTelemetry {
		effectiveResyncPeriod = 60
		setupLog.Info("instance telemetry enabled: overriding resync period to 60s")
	}
	...
	dc := dynamiccontroller.NewDynamicController(rootLogger, dynamiccontroller.Config{
		...
		ResyncPeriod:    time.Duration(effectiveResyncPeriod) * time.Second,
