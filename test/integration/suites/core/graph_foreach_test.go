...
var _ = Describe("Graph ForEach", func() {
	...
	It("computes cartesian product for multi-dimension forEach", func() {
		t := GinkgoT()
		ns := env.CreateNamespace(t)
		...
		want := []string{"sq-g", "sq-r", "tri-g", "tri-r"}
		sort.Strings(want)
		environment.Eventually(t, 15*time.Second, 100*time.Millisecond, func() error {
			got, err := listConfigMapNames(env, ns)
			if err != nil {
...
func listConfigMapNames(env *environment.Environment, ns string) ([]string, error) {
