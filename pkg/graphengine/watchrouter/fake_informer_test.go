...
func (f *fakeInformer) HasSyncedChecker() cache.DoneChecker {
	return fakeDoneChecker{name: "fakeInformer", synced: f.HasSynced()}
}
...
func (f *fakeReg) HasSyncedChecker() cache.DoneChecker {
	return fakeDoneChecker{name: "fakeReg", synced: true}
}

// fakeDoneChecker satisfies cache.DoneChecker for the test fakes. Done()
// returns an already-closed channel when synced, otherwise a channel that
// never closes.
...
func (c fakeDoneChecker) Name() string { return c.name }
