...
func (t *transformer) buildFieldFromString(name, fieldValue string, parent *extv1.JSONSchemaProps, path []string) (*extv1.JSONSchemaProps, []PrinterColumn, error) {
	...
	if len(customTypesPrinterColumns) > 0 {
		if printConfig.Enabled && len(customTypesPrinterColumns) > 1 {
			return nil, nil, fmt.Errorf("kubectlPrint on field %q references custom type with %d printer columns; title override is only allowed when the custom type produces exactly one column", name, len(customTypesPrinterColumns))
		}
		overrideTitle := ""
		if printConfig.Enabled && len(customTypesPrinterColumns) == 1 {
...
func (t *transformer) printerColumnsForType(typ types.Type) ([]PrinterColumn, error) {
	...
	case types.Slice:
		...
		if len(printerColumns) > 0 {
			return nil, fmt.Errorf("kubectlPrint markers inside array types are not supported")
		}
		return nil, nil
	case types.Map:
		...
		if len(printerColumns) > 0 {
			return nil, fmt.Errorf("kubectlPrint markers inside map types are not supported")
		}
		return nil, nil
	default:
		return nil, nil
