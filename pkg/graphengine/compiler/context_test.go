...
func TestCompilationContext_BuildNode(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// A ref that sets BOTH name and selector must be rejected at compile
			// time on the inline-Graph path — the CRD XValidation only guards
			// API-server admission, so a directly-compiled Graph would otherwise
			// silently drop the name and list by selector.
			name: "static ref with both name and selector is rejected",
			node: &expv1alpha1.Node{ID: "p", Ref: &expv1alpha1.ExternalRef{
				APIVersion: "v1", Kind: "Pod",
				Metadata: expv1alpha1.ExternalRefMetadata{
					Name:     "p",
			...
			wantErr: "exactly one of name or selector",
		},
		{
			// A ref with NEITHER name nor selector would build a broken empty-name
			// read; reject it at compile time instead.
			name: "static ref with neither name nor selector is rejected",
			node: &expv1alpha1.Node{ID: "p", Ref: &expv1alpha1.ExternalRef{
				APIVersion: "v1", Kind: "Pod",
			}},
			wantErr: "exactly one of name or selector",
		...
		{
			name: "dynamic ref with both name and selector is rejected",
			node: &expv1alpha1.Node{ID: "dynref", Ref: &expv1alpha1.ExternalRef{
				APIVersion: "${cfg.group}/v1", Kind: "Widget",
				Metadata: expv1alpha1.ExternalRefMetadata{
					Name:     "w",
		...
		{
			name: "dynamic ref with neither name nor selector is rejected",
			node: &expv1alpha1.Node{ID: "dynref", Ref: &expv1alpha1.ExternalRef{
				APIVersion: "${cfg.group}/v1", Kind: "Widget",
			}},
			wantErr: "exactly one of name or selector",
